    ...
    case "$store" in
        keychain)
            if [ -n "$kc_account" ]; then
                local blob
                blob=$(security find-generic-password -s "Claude Code-credentials" -a "$kc_account" -w 2>/dev/null)
                if [ -n "$blob" ]; then
                    local updated
                    ...
                    security delete-generic-password -s "Claude Code-credentials" -a "$kc_account" >/dev/null 2>&1
                    security add-generic-password -s "Claude Code-credentials" -a "$kc_account" -w "$updated" >/dev/null 2>&1
                fi
            fi
            ;;
        file)
...
try_keychain_account() {
    ...
    blob=$(security find-generic-password -s "Claude Code-credentials" -a "$acct" -w 2>/dev/null) || return 1
    [ -z "$blob" ] && return 1
...
get_oauth_token() {
    ...
    #    - Newer: OS username (e.g. "florianriquelme")
    #    - Older: "Claude Code"
