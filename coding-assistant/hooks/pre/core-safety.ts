#!/usr/bin/env node
/**
 * core-safety.ts — Oh My Pi pre-tool-call safety hook.
 *
 * Ports the Claude Code protection stack to OMP:
...
function loadPatterns(): PatternConfig {
  ...
  const globalConfig = loadPatternConfig(
    path.join(os.homedir(), ".claude", "security", "denylist.json")
  );
  if (globalConfig) {
  ...
  const projectConfig = loadPatternConfig(
    path.join(process.cwd(), ".claude", "security", "denylist.json")
  );
  if (projectConfig) {
...
function loadOverrides(): { deny: Set<string>; allow: Set<string> } {
  ...
  addPatterns(deny, loadJsonArray(path.join(os.homedir(), ".claude", "security", "deny.json")));
  addPatterns(allow, loadJsonArray(path.join(os.homedir(), ".claude", "security", "allow.json")));
  return { deny, allow };
}
...
function checkSecretProtection(event: ToolCallEvent): BlockResult | undefined {
  ...
  if (["read", "edit", "write"].includes(toolName)) {
    ...
    const grepPath = inputString(event.input, ["path"]);
    if (grepPath) pathsToCheck.push(grepPath);
  } else if (toolName === "glob") {
    const pattern = inputString(event.input, ["pattern"]);
    const globPath = inputString(event.input, ["path"]);
    if (pattern) pathsToCheck.push(pattern);
    if (globPath) pathsToCheck.push(globPath);
  }
